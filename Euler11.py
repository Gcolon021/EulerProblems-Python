...
if __name__ == '__main__':
    ...
    values = map(int, numbers.split())
    numberList = list(values)
    print(numberList)
    answer = 0
    for x in range(397):
        # left <-> right
        product = numberList[x] * numberList[x + 1] * numberList[x + 2] * numberList[x + 3]
        if product > answer: answer = product
    print(product)
    for z in range(0, 340):
        product = numberList[z] * numberList[z + 20] * numberList[z + 40] * numberList[z + 60]
        if product > answer: answer = product
    print(product)
    for i in range(0, 337):
        # Diag --- Top Left Bottom Right
        product = numberList[i] * numberList[i + 21] * numberList[i + 42] * numberList[i + 63]
        if product > answer: answer = product
    print(product)
    for j in range(3, 340):
        # Diag --- Top right to Bottom Left
        product = numberList[j]*numberList[j+19]*numberList[j+38]*numberList[j+57]
        if product > answer: answer = product
    print(product)
    print("Answer: ", answer)
